...
compare <- current %>%
  filter(!grepl("EMERGENCY", EXTERNAL_NAME),
         is.na(VALID_TO)) %>%
  full_join(new, 
...
check <- tbl(con_prod, "OE_CHARGE_DETAIL") %>%
  filter(BILLING_CAT_DESC == 'BED CHARGES',
         SERVICE_DATE >= 20240601) %>%
  filter(!(EPIC_DEPT_ID %in% c("8503016", "8503017"))) %>% # manually remove MSW L&D
  select(EPIC_DEPT_ID) %>%
  distinct() %>%
  collect() %>%
...
### updates Mapping
```{r updates changes, include=FALSE}
...
if (nrow(change) > 0) {
  ...
  # updates current mappings and combine with changes
  updates <- current %>%
    filter(EPIC_DEPT_ID %in% change$EPIC_DEPT_ID) %>%
    mutate(VALID_TO = Sys.Date() - 1) %>%
    rbind(change)
  
  # replace updatess and changes in current mappings
  updates <- current %>%
    filter(!(EPIC_DEPT_ID %in% updatess$EPIC_DEPT_ID)) %>%
    rbind(updatess)
