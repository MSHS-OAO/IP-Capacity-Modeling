...
```{r dummy new data, include=FALSE}
new_id <- c(11111, "TEST", "TEST", "TEST")
new_id2 <- c(22222, "TEST", "TEST", "TEST")

new <- new %>%
  rbind(new_id, new_id2) %>%
  mutate(SERVICE_GROUP = case_when(
    DEPARTMENT_ID == 8505109 ~ "TEST",
    DEPARTMENT_ID == 8505110 ~ "TEST",
    TRUE ~ SERVICE_GROUP),
...
#####################################
check <- data.frame(EPIC_DEPT_ID = c(11111, 22222))
#####################################
