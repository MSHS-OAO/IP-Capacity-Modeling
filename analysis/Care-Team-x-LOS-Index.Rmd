...
encounter_care_team <- baseline %>%  
  ...
  mutate(PROVIDER_CHANGE = NPI != lag(NPI),
         NEW_PROVIDER    = is.na(lag(NPI)) | PROVIDER_CHANGE,
         PROVIDER_ID     = cumsum(NEW_PROVIDER),
         
         CARE_TEAM_CHANGE = SERVICE_TEAM != lag(SERVICE_TEAM),
         NEW_CARE_TEAM    = is.na(lag(SERVICE_TEAM)) | CARE_TEAM_CHANGE,
...
encounter_care_switch <- encounter_care_team %>%
  ...
  summarise(PROVIDER_COUNT = max(PROVIDER_ID),
            CARE_TEAM_COUNT = max(CARE_TEAM_ID))
...
no_outliers <- encounter_care_switch %>%
  filter(CARE_TEAM_COUNT <= 15,
         LOS_INDEX <= 10) %>%
  mutate(CARE_TEAM_COUNT_F = factor(CARE_TEAM_COUNT, levels = 1:15, ordered = TRUE)) %>%
...
plot_main <- ggplot(no_outliers, aes(x = CARE_TEAM_COUNT_F, y = LOS_INDEX)) + 
  ...
  labs(title = "LOS Index by Care Team Count",
       subtitle = "Care Team Count <= 15; LOS Index <= 10",
       y = "LOS Index",
       x = NULL) + 
  theme(
...
plot_table <- ggplot(n_counts, aes(x = CARE_TEAM_COUNT_F, y = 1)) +
  geom_text(aes(label = label), size = 3, color = "black", fontface = "plain") + 
