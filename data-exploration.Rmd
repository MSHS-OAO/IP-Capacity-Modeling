...
```
# Data Exploration
### Check for unique encounters
```{r unique encounters}
# check if each patient encounter is unique
unique_encounters <- if (length(unique(MSX_IP_OUTPUT$ENCOUNTER_NO)) == nrow(MSX_IP_OUTPUT)) {
  print("All encounters are unique")
} else {
  print("There are duplicate encounters")
}
```
...
ggplot(MSX_IP_OUTPUT, aes(x = FACILITY_MSX, fill = YEARMO_MSX)) +
  geom_bar(position = "dodge", color = "black", size = 0.2)
```
...
current <- MSX_IP_OUTPUT %>%
  ...
  filter(FACILITY_MSX %in% c("MSH", "STL"),
...
scenario <- current %>%
  mutate(
    FACILITY_MSX = case_when(
      ...
      SERVICE_DESC_MSX == 'VASCULAR SURGERY' ~ "STL"),
    STATE = "FUTURE") 
