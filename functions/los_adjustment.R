los_reduction_sim <- function(encounter_days_df) {
  ...
  baseline_los <- encounter_days_df %>%
    ...
    distinct(ENCOUNTER_NO, DSCH_UNIT_DESC_MSX, ATTENDING_VERITY_REPORT_SERVICE, LOS_NO_SRC) %>%
    group_by(DSCH_UNIT_DESC_MSX, ATTENDING_VERITY_REPORT_SERVICE) %>%
    summarise(ALOS = mean(LOS_NO_SRC, na.rm=TRUE),
              .groups = "drop") %>%
    mutate(UNIQUE_ID = paste0(DSCH_UNIT_DESC_MSX, ATTENDING_VERITY_REPORT_SERVICE))
