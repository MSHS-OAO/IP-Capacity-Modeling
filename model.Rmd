...
for (i in 1:length(datasets_processed)) {
  lab_demand[[i]] <- datasets_processed[[i]] %>%
    ...
    filter(LAB_COUNT == 1) %>%
    group_by(FACILITY_MSX, SERVICE_MONTH, SERVICE_DATE, CPT_HCPCS_C, DESCRIPTION_LONG) %>%
...
for (i in 1:length(datasets_processed)) {
  rad_demand[[i]] <- datasets_processed[[i]] %>%
    filter(FACILITY_MSX %in% hospitals,
           BILLING_CAT_DESC %in% (charge_cat %>% 
                                    filter(CATEGORY == "RAD") %>%
...
lab_rad_baseline_comp <- datasets_processed[["baseline"]] %>%
  filter(FACILITY_MSX %in% hospitals) %>%
  left_join(cpt_map, by = c("CPT_HCPCS_C"="CPT")) %>%
  filter(LAB_COUNT == 1) %>%
