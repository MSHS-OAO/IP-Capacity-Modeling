...
total_utilization <- ip_comparison_daily %>%
  group_by(FACILITY_MSX, SERVICE_DATE) %>%
  summarise(BASELINE = sum(DAILY_DEMAND_BASELINE, na.rm = TRUE)/sum(AVG_BED_CAPACITY_BASELINE, na.rm = TRUE),
            SCENARIO = sum(DAILY_DEMAND_SCENARIO, na.rm = TRUE)/sum(AVG_BED_CAPACITY_SCENARIO, na.rm = TRUE)) %>%
